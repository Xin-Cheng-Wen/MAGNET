...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, help='Name of the dataset for experiment.', default='Fan')
    parser.add_argument('--input_dir', type=str, help='Input Directory of the parser', default='../fan_type1')
    ...
    processed_data_path = os.path.join(input_dir, 'hgt_c1_1_fan.bin')
    logging.info('#' * 100)
    if True and os.path.exists(processed_data_path):
        ...
        dataset = DataSet(train_src=os.path.join(input_dir, './fan-train-v0.json'),
                          valid_src=os.path.join(input_dir, './fan-valid-v0.json'),
                          test_src=os.path.join(input_dir, './fan-test-v0.json'),
    ...
    loss_function = CrossEntropyLoss(weight=torch.from_numpy(np.array([1,16.0])).float(),reduction='sum')
    loss_function.cuda()
    #loss_function = CrossEntropyLoss(reduction='sum')
    LR = 5e-4
    optim = Adam(model.parameters(), lr=1e-3, weight_decay=1.3e-6)
    ...
    train(model=model, dataset=dataset, epoches=100, dev_every=len(dataset.train_batches),
          ...
          save_path=model_dir + '/DevignModel', max_patience=100, log_every=5)  #models/FFmpeg/GGNNSumModel....
