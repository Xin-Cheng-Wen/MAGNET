...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', type=str, help='Input Directory of the parser', default='../devign_type1')
    ...
    processed_data_path = os.path.join(input_dir, 'hgt_c1_512.bin')
    logging.info('#' * 100)
    ...
    loss_function = CrossEntropyLoss(weight=torch.from_numpy(np.array([1,1])).float(),reduction='sum')
    loss_function.cuda()
    ...
    train(model=model, dataset=dataset, epoches=100, dev_every=len(dataset.train_batches),
          ...
          save_path=model_dir + '/DevignModel', max_patience=30, log_every=5)  #models/FFmpeg/GGNNSumModel....
